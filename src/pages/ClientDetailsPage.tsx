...
import DebtCyclesModal from "@/components/clients/DebtCyclesModal";
...
import { computeOpeningBalance, computeDebtCycles } from "@/utils/computeDebtCycles";
...
const ClientDetailsPage: React.FC<ClientDetailsPageProps> = ({
  ...
  const handleExportPDF = async () => {
    ...
    // --- DATA PREPARATION (CYCLE-BASED) ---
    const chronologicalAll = [...allClientTransactions].sort((a, b) => {
    ...
    const historyOpeningBalance = computeOpeningBalance(chronologicalAll, currentBalance);
    const allCycles = computeDebtCycles(chronologicalAll, historyOpeningBalance);
    ...
    allCycles.forEach((cycle, cycleIndex) => {
      ...
      const isCycleDebt = cycleOpeningBalance < 0;
      const isCycleCredit = cycleOpeningBalance > 0;
      ...
      let runningTotalForGrand = isCycleDebt ? absOpenBal : 0;

      // B/F Debt box (only when cycle 1 carries a prior debt)
      if (isCycleDebt) {
        ...
        doc.setTextColor(51, 51, 51);
        doc.text(
          `B/F Debt - ${formatDate(periodStart)}: ${formatCurrencyForPDF(absOpenBal)}`,
          margin + 4,
          cursorY + 6.5
        );
        cursorY += 18;
      ...
      cycleSupplies.forEach((txn) => {
        ...
        if (discrepancy > 1) charges.push({ label: "Other Charges", amount: discrepancy });
        ...
        if (t.items && t.items.length > 0) {
          ...
          doc.text(formatCurrencyForPDF(itemsTotal), pageWidth - margin - 4, cursorY, { align: "right" });
          cursorY += itemLineHeight;
        }
        ...
        if (discount > 0) {
          ...
          doc.text(`-${formatCurrencyForPDF(discount)}`, pageWidth - margin - 4, cursorY, { align: "right" });
      ...
      // Credit opening balance shows in Less (only cycle 1 can have this)
      if (isCycleCredit) {
        const itemHeight = 18;
        ...
        doc.setFontSize(8);
        doc.setTextColor(51, 51, 51);
        doc.text(`On ${formatDate(periodStart)}`, margin + 7, cursorY + 6);
        doc.setFontSize(9);
        const creditDesc = "Opening Balance (User Credit): ";
        const creditVal = formatCurrencyForPDF(absOpenBal);
      ...
      cycleTxns.forEach((txn) => {
        const t = txn as any;
        const displayItems: { description: string; value: string; isReturn: boolean }[] = [];
        ...
        if (t.type === "DEPOSIT") {
          ...
          if (t.items && t.items.length > 0) {
            ...
            t.items.forEach((item: any) => {
              ...
              doc.text(formatCurrencyForPDF(isSub ? amount : rate).slice(1), retColRate, cursorY, { align: "right" });
              doc.text(formatCurrencyForPDF(amount).slice(1), retColAmount, cursorY, { align: "right" });
      ...
      // --- BALANCE (per cycle) ---
      checkPageBreak(12);
      let cycleBalanceLabel = "BALANCE";
      if (cycleFinalBalance < 0) cycleBalanceLabel = "BALANCE (DEBT)";
      else if (cycleFinalBalance > 0) cycleBalanceLabel = "BALANCE (CREDIT)";

      const cycleBalanceTextColor: [number, number, number] =
        cycleFinalBalance > 0 ? [46, 204, 113] : [204, 0, 0];
      const cycleBalanceFillColor: [number, number, number] =
        cycleFinalBalance > 0 ? [232, 250, 240] : [252, 240, 242];
      const cycleBalanceBorderColor: [number, number, number] =
        cycleFinalBalance > 0 ? [46, 204, 113] : [204, 0, 0];

      doc.setFillColor(...cycleBalanceFillColor);
      doc.rect(margin, cursorY, pageWidth - margin * 2, 12, "F");
      doc.setFillColor(...cycleBalanceBorderColor);
      doc.rect(margin, cursorY, 1.5, 12, "F");
      doc.setFont("helvetica", "bold");
      doc.setFontSize(11);
      doc.setTextColor(...cycleBalanceTextColor);
      doc.text(
        `${cycleBalanceLabel}: ${formatCurrencyForPDF(cycleFinalBalance)}`,
        pageWidth - margin - 4,
        cursorY + 8,
        { align: "right" }
      );
  ...
  return (
    <>
      ...
      <DebtCyclesModal
        isOpen={showCyclesModal}
        onClose={() => setShowCyclesModal(false)}
        allClientTransactions={allClientTransactions}
